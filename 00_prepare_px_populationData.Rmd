...
df <- data.read %>% filter(
  ...
  year == "2015"
) %>% select(nationality, location, value)
...
### by commune compute % foreigners and the top 6 foreign nationalities
topN <- df %>% group_by(nationality) %>% summarise(tot = sum(value)) %>% 
  arrange(desc(tot)) %>% ungroup() %>% head(7) %>%
  select(nationality) %>% unlist(use.names = F)

ddd <- df %>% group_by(location, BFS_NUMMER) %>%
  summarise(
    population   = sum(value),
    foreigners   = sum(value[nationality != "Suisse"]),
    pcForeigners = round(sum(foreigners) / population, 3),
    Italie       = round(sum(value[nationality == "Italie"]) / foreigners, 3),
    Allemagne    = round(sum(value[nationality == "Allemagne"]) / foreigners, 3),
    Portugal     = round(sum(value[nationality == "Portugal"]) / foreigners, 3),
    France       = round(sum(value[nationality == "France"]) / foreigners, 3),
    Kosovo       = round(sum(value[nationality == "Kosovo"]) / foreigners, 3),
    Espagne      = round(sum(value[nationality == "Espagne"]) / foreigners, 3),
    Autres       = round(sum(value[!nationality %in% topN] / foreigners), 3)
  ) %>% ungroup()
