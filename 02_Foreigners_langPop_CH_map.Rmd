...
```{r settings}
translation.file <- "input/Map foreigners, language and population density - Sheet1.csv"
...
for (lang in colnames(txt)) {
  ...
  ddd$tip <- paste0(
    '<h4>', as.character(ddd$GEMNAME), '</h4><div class="tp">', txt['for.tp', lang], 
    ' <b>', ddd$pcForeigners, '%</b><br>(',
    ifelse(ddd$deltaForeigners>0, "+", ""), signif(ddd$deltaForeigners, 3) * 100, 
    " ", txt['pc.tp', lang], ")",
    '<br>', txt['pop.tp', lang], ': ', ddd$population
  )
  ddd$tip <- gsub("'", "_", gsub("\\\n", "", ddd$tip))
  ...
  ### Write the non-interactive map as images
    ...
    map3.path <- paste0("output/map_pop_" , lang, ".png")      
    ...
    png(map3.path,  res = 200, pointsize = 1, height = 800 * 1.1, 
        width = 1100 * 1.1, bg = '#1a0000')
    print(map.pop)
    dev.off()   
    ...
    save_html(
      tags$html(
        ...
        tags$body(    
          ...
          h3(txt["title4", lang]),
          div(class = "descr", HTML(txt["descr4", lang])),
          div(img(src = basename(map3.path), width= "100%")),
          HTML(iframeresizer)  
