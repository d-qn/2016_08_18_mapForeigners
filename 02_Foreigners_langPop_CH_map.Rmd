...
for (lang in colnames(txt)) {
  ...
  map2 <- ggplot(co.df) + 
    ...
    nats <- c('Italie', 'Allemagne', 'Portugal', 'France', 'Kosovo', 'Espagne', 'Autres')
    ddd[is.na(ddd)] <- 0
    max_bar <- 100
    
    ddd$tip <- paste0(
      '<b>', as.character(ddd$GEMNAME), '</b><br>',
      '<b>', ddd$pcForeigners * 100, '%</b>, population: ', ddd$population, '<br><br>',
      '<i>Nationalité des étrangers en %</i><div class="tipchart"><table>'
      )
    
    tiprowize <- function(name, value) {
      value <- value * 100
      sprintf(
      '<tr class="tiprow">
        <td class="tipbarticks">%s</td>
        <td class="tipbardiv"><div class="tipbar" style="width:%dpx;">%3.1f</div></td>
      </tr>',
      name,
      round((value/(max_bar/1.25)*100)), 
      value
      )
    }
    trow <- sapply(nats, function(nat) tiprowize(rep(nat, nrow(ddd)), ddd[,nat]))
    ddd$tip <- paste0(ddd$tip, apply(trow, 1, paste0, collapse = ""))
    ddd$tip <- paste0(ddd$tip, "</table></div>")
    ddd$tip <- gsub("'", "_", gsub("\\\n", "", ddd$tip))
    ...
    imap <- ggiraph(code = {print(fmap)}, width = "100%", zoom_max = 1)
