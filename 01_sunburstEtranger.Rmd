...
```{r load data}
# load translations
txt <- read.csv("input/Sunburst CH foreigners 2016 - Sheet1.csv", row.names = 1, stringsAsFactors = F)
...
if(loadPXdata) {
  ...
  data.read %<>% filter(Jahr ==  max(data.read$Jahr)) %>%
    ...
    rename(nationality = `Staatsangehörigkeit`) %>%
    select(nationality, value)
    
  # get the English nationality 
  nationality <- gsub('\"', '', unlist(strsplit(en.metadata$Citizenship, ",")))
...
```{r wrangle & structure data}

# Remove Suisses!
data <- filter(data, data$nationality != "Switzerland", value > 0) 
  ...
  save_html(
    tags$html(
      ...
      tags$body(
        ...
        div(class = "descr", HTML(paste0(txt['descr', lang], ' &nbsp; &nbsp; <img src="https://interactive.swissinfo.ch/assets/Interactive_icon.svg.png" width="19" align="top">'))),
